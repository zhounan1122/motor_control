...
// InitEQep:
//---------------------------------------------------------------------------
// 采样频率4KHz，每250us便会把QPOSCNT的值锁存在QPOSLAT中，
void InitEQep(void)
{
	InitEQep1Gpio();

	#if (CPU_FRQ_150MHZ)
	  EQep1Regs.QUPRD=37500;			// Unit Timer for 4KHz at 150 MHz SYSCLKOUT
	#endif
    #if (CPU_FRQ_100MHZ)
	  EQep1Regs.QUPRD=25000;			// Unit Timer for 4KHz at 100 MHz SYSCLKOUT
	#endif

	EQep1Regs.QDECCTL.bit.QSRC=00;		// QEP quadrature count mode

	EQep1Regs.QEPCTL.bit.FREE_SOFT=2;
	EQep1Regs.QEPCTL.bit.PCRM=11;		// PCRM=11 mode - QPOSCNT reset on unit time event
